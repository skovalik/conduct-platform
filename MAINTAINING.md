...
- **`scope` does not drive path selection.** The emitted layout is always project-shaped under `root`; per-harness global layouts are unbuilt. `docs/cli.md` documents project scope as the supported path; treat global-scope support as a design decision to make deliberately, not a quick fix.
