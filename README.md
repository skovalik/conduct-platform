...
Under it: operating rules that make the model plan before it codes and verify before it claims done, layered memory so context survives between sessions, and the supporting tools, generated into each tool's native format. You start from a calibrated workspace instead of a blank prompt.

The goal is **transfer, not install**: you should be able to plan, research, and implement with real context engineering from day one, not just have files emitted. "Done" is a user outcome, not "it installed."
...
- **The memory rule set**: the on-demand "how the agent thinks and verifies" files, cross-linked and loaded only when referenced.
- **The supporting tools**: cross-session recall, an issue tracker, deep-research, and the rest, wired up per harness.
- **The habit**: a guided first run that demonstrates the loop once, with you.
...
3. **The engine.** conduct-platform's own, platform-agnostic value: the interactive bootstrap, the lifecycle and state engine, the dependency manifest with per-OS detection, the memory scaffold with exemplars, and the transfer and onboarding layer.

## How it works (install flow)

1. You run setup on your harness. It detects your OS, your harness, and your installed tooling (names only, never secrets).
